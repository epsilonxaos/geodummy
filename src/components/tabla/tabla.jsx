...
import {MdOutlineMoreVert, MdAutorenew, MdWarning, MdDeleteForever} from 'react-icons/md';
...
const TrList = ({addMarker, id, estado, tipo, referencia, etapa, location, active}) => {
	...
	return(
		<tr className={` border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 ${id == active ? 'bg-gray-100 dark:bg-gray-600' : 'bg-white'}`}>
			...
			<td className="px-6 py-4" onClick={(ev) => {ev.stopPropagation(); addMarker(location, id);}}>
				<span className="font-semibold hover:text-sky-600 cursor-pointer">{referencia}</span>
			</td>
			<td className="px-6 py-4">
				{moment().format('L')}
			</td>
				...
				<Dropdown>
					...
					<Dropdown.Body>
						<ul className="text-sm text-gray-700 dark:text-gray-200">
							<li>
								<span className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"><MdAutorenew className="inline-block mr-1" size={"18px"} /> Cambiar estado</span>
							</li>
							<li>
								<span className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"><MdWarning className="inline-block mr-1" size={"18px"} /> Reportar</span>
							</li>
							<li>
